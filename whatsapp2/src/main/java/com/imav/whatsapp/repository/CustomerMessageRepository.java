...
public interface CustomerMessageRepository extends JpaRepository<CustomerMessage, String> {
	...
	@Query(value = "SELECT COUNT(m_phone_from) FROM customer_message c WHERE c.m_phone_from = :m_phone_from", nativeQuery = true)
	Integer countByM_Phone_From(@Param("m_phone_from") String m_phone_from);
