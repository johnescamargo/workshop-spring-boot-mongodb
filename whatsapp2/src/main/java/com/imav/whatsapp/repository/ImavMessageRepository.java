...
public interface ImavMessageRepository extends JpaRepository<ImavMessage, Long> {
	...
	ImavMessage findByIdWamid(@Param("id_wamid") String idWamid);

	Boolean existsByIdWamid(@Param("id_wamid") String id_wamid);

	@Query(value = "SELECT COUNT(m_to) FROM imav_message c WHERE c.m_to = :m_to", nativeQuery = true)
	Integer countByM_to(@Param("m_to") String m_to);
