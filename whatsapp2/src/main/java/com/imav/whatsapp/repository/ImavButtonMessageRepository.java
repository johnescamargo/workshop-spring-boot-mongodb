...
public interface ImavButtonMessageRepository extends JpaRepository<ImavButtonMessage, Long> {
	...
	ImavButtonMessage findByPhoneAndIdWamid(@Param("b_to") String b_to, @Param("id_wamid") String id_wamid);

	Boolean existsByIdWamid(@Param("id_wamid") String id_wamid);

	ImavButtonMessage findByIdWamid(@Param("id_wamid") String idWamid);

	@Query(value = "SELECT COUNT(b_to) FROM imav_buttonmessage c WHERE c.b_to = :b_to", nativeQuery = true)
	Integer countByB_to(@Param("b_to") String b_to);
